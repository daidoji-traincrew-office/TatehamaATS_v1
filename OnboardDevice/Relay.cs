...
namespace TatehamaATS_v1.OnboardDevice
{
    ...
    internal partial class Relay
    {
        ...
        public async Task ForceStopSignal(bool IsStop)
        {
            TrainCrewInput.GetTrainState();
            foreach (var signalData in TrainCrewInput.signals.ToList())
            {
                ...
                await SetSignalPhase(signalData.name, phase);
            }
        }
        ...
        internal async Task SetSignalPhase(string signalName, Phase phase)
        {
            if (signalName == "上り1閉塞")
            {
                return;
            ...
            Debug.WriteLine($"☆信号名：{signalName}／現示：{phase.ToString()}");
            await SendSingleCommand("SetSignalPhase", new[] { signalName, phase.ToString() });
        }
